// NetLog
// Used to log data to various places (commandSerial, HTTP) from ArduinoLog module
// Rob Dobson 2018
...
void NetLog::setup(ConfigBase *pConfig, const char* systemName)
{
    ...
    // Debug
    if (_logToSerial && _serialPort == 0)
        Serial.printf("NetLog: logLevel %d, httpFlag %d, ip %s, port %d, url %s, serialFlag %d, serialPort %d, cmdSerial %d, papertrailFlag %d, host %s, port %d\n",
                _loggingThreshold,
...
size_t NetLog::write(uint8_t ch)
{
    ...
    // Check for log to HTTP
    if (!(_logToHTTP || _logToCommandSerial || _logToPapertrail))
        return retVal;
